...
# Custom Comments - This script is an extended version of network-config.sh
# It displays a one-time host report of network configuration
# Then is displays individual interfaces network configuration report using a for loop
...
########## New - Get the list of interfaces except loopback
interfaces=$(ip -o -4 a | awk '{print $2}' | tail -n +2)

########## New - Start the for loop for every interface found
for interface in $interfaces; do
  ...
  network_address=$(ip route list dev $interface proto kernel scope link|cut -d ' ' -f 1)
