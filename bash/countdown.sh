...
# Custom Comments
# This script displays a countdown window which requires 'dialog' to be installed on your system (sudo apt install dialog)
# The default values of coutdown are 1 sec wait and 10 sec total time
# The default values can be changed using command line options
# To know the usage, use the script with option -h|--help to display help
# On using INT or ^C while coutdown is running, it restarts as you can't interrupt it
# On using Quit or ^\ while coutdown is running, exits immediately. This is a secret to exit the coutdown
...
function trappedINT {
        echo "You're not allowed to interrupt"
        sleepCount=$numberOfSleeps
        doCountdown|dialog --gauge "Remaining Time" 7 60
}

function trappedQUIT {
  echo "Congratulations! You found the secret to getting out of the script."
  exit
}

trap trappedINT SIGINT
trap trappedQUIT SIGQUIT
